...
it('ecdh test', () => {
  ...
  const message = '中文1234';
  let res = Aes256Gcm.encrypt(message, Buffer.from(share21));
  const iv = res.slice(0, 16);
  const tag = res.slice(16, 32);
  const aad = res.slice(32, 48);
  const ciphertext = res.slice(48);
  let cleartext = Aes256Gcm.decrypt(ciphertext, iv, aad, share12, tag);
