...
const ETH_MSG_PREFIX = '\u0019Ethereum Signed Message:\n';
...
function getPublicKeyFor(data: Buffer, signature: Buffer) {
  const prefix = Buffer.from(`${ETH_MSG_PREFIX}${data.length}`, 'utf-8');
  const msgHash = keccak(Buffer.concat([prefix, data]));
  // console.log("getPublicKeyFor", {msgHash})
  // console.log("msg_hash_hex", bufferToHex(msgHash))
...
export const pubToAddress = function (
  ...
  console.log('pubToAddress keccak(pubKey)', hash.length, hash);
...
export default class EcdsaHelper {
  ...
  static recoverAddress({ sig, message }: { sig: Buffer; message: string }) {
    ...
    console.log('recoverAddress publicKey', publicKey.length, publicKey);
    console.log('recoverAddress publicKey hex', bufferToHex(publicKey));
    const sender = publicToAddress(publicKey);
    const senderHex = bufferToHex(sender);
    console.log('recoverAddress', sender.length, sender, senderHex);
    return senderHex;
  }
}
