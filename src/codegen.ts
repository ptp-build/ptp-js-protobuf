...
export class SimpleCodeGen {
    ...
    public pushLine(line: string = '', ...values: any[]): SimpleCodeGen {
        if (line) {
            let counter = 0;
            line = line.replace(/%([%dfsjk])/g, (_, $1) => {
                let value = values[counter ++];
                switch ($1) {
                    case 'd': case 'f': return String(Number(value));
                    case 's': return String(value);
                    case 'j': return JSON.stringify(value);
                    case 'k': return JSON.stringify(value,null,4);
                }
                return '%';
            });
            ...
            this._lines.push(`${this._indentStyle.repeat(this._indentCount < 0 ? 0: this._indentCount)}${line}`);
        } else {
            this._lines.push('');
        }
        return this;
